# How to activate the background Noise cancellation?

You are participating to an ongoing session but some unwanted noise **in your background** is disturbing the communication. ![](../../../.gitbook/assets/Apizee-noise-cancellation.png)

1. On the right, click the **Settings** tab.
2. Click **Audio & video**.
3. Switch the button in front of **Noise cancellation**.

![](../../../.gitbook/assets/EN-conf-settings-noise-cancellation.png)

{% hint style="success" %}
As a result, you experience a reduced level of surrounding noise during the session.
{% endhint %}
