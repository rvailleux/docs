# join-and-leave-the-conference-as-a-guest

## Join the session

{% hint style="info" %}
You received a text message or an email with a link to join the session.
{% endhint %}

1. Click the **link**in the message.
2. Enter your name then, click **Confirm**.

```
| ![Information](../../.gitbook/assets/info.png) | If you have an Apizee account, click **Log in**. |
| --- | --- |
```

2\. Tick the boxes to agree to the terms then, click **Confirm**.

&#x20;3\. **Allow**your web browser to use the microphone and the camera. If you do not allow, the participants will not hear you and see you.

&#x20;4\. Get ready, check your microphone and camera then, click **Join conference**. If you want it, you can turn off the microphone and the camera before joining the session. You can turn them on again later.

**Want to change your background?**

|   | **See also** [Change the virtual background](../configure-my-video-conference-settings/change-the-virtual-background.md) |
| - | ------------------------------------------------------------------------------------------------------------------------ |

\[+] [Show More](https://github.com/rvailleux/docs/tree/master/faq/meetings/guests/start-a-video-conference/javascript:void\(0\)/README.md) \[-] [Hide](https://github.com/rvailleux/docs/tree/master/faq/meetings/guests/start-a-video-conference/javascript:void\(0\)/README.md) |  | If you want to join the conference with a phone or by QR code, click **More**.\
Here are the different options: | | --- | --- |

{% hint style="success" %}
You are in the waiting room.
{% endhint %}

Wait for the organizer to allow you to enter into the session.

The organizer has been informed and he/she can send you messages to keep you informed. | | --- | --- |

{% hint style="success" %}
The communication starts.
{% endhint %}

## Leave the session

When the appointment is over, click&#x20;
